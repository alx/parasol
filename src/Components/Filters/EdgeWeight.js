...
export default class EdgeWeight extends Component {
  ...
  render() {
    ...
    let chartData = {
      labels: [],
      datasets: [{ data: [], backgroundColor: "#00BCD4", borderWidth: 0 }]
    };
    ...
    if (graph.minEdgeWeight) {
      ...
      const resolution = parseInt(
        (graph.maxEdgeWeight - graph.minEdgeWeight) / graph.edgeWeightStep
      );
      ...
      graph.edges.map(e => e.weight).forEach(weight => {
        const dataPosition = parseInt(
          (weight - graph.minEdgeWeight) / graph.edgeWeightStep
        );
    ...
    return (
      <div
        key={"filterEdgeWeight-" + appState.graph.refresh}
        style={{ padding: 10 }}
      >
        <p>
          <span>Edge Weight</span>
        </p>
        <Bar data={chartData} height={50} options={chartOptions} />
        <Range
          key={"edgeWeight"}

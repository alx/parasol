...
export default class NodeItem extends Component {
  render() {
    ...
    const isFiltered =
      appState.ui.componentOptions && appState.ui.componentOptions.selectedNode;
    ...
    if (!isMetadata) {
      ...
      if (!node.metadata[key]) return null;
    ...
    if (isMetadata) secondaryText = `metadata - ${key}`;

    if (isFiltered) {
      const fieldConfig = options.content.find(f => {
        return isMetadata
          ? f.field.replace("metadata.", "") == key
          : f.field == key;
      });

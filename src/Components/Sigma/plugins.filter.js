      ...
      (function() {
        (function(undefined) {
          ...
          Processors.nodes = function nodes(fn) {
            ...
            while (le--)
              if (
                _g.nodes(e[le].source).hidden ||
                _g.nodes(e[le].target).hidden
              )
                e[le].hidden = true;
          };
          ...
          Filter.prototype.undo = function(v) {
            ...
            if (la === 1) {
              if (Object.prototype.toString.call(v) === "[object Array]")
                for (var i = 0, len = v.length; i < len; i++) q[v[i]] = true; // 1 filter key
              else q[v] = true;
            } else if (la > 1) {
          ...
          function cloneChain(chain) {
            ...
            for (var i = 0, len = copy.length; i < len; i++) {
              ...
              if (typeof copy[i].processor === "function")
                copy[i].processor =
                  "filter.processors." + copy[i].processor.name;
            }
            return copy;
          }
          ...
          Filter.prototype.import = function(chain) {
            ...
            for (var i = 0, len = copy.length; i < len; i++) {
              ...
              if (typeof copy[i].predicate !== "function")
                throw 'The predicate of key "' +
                  copy[i].key +
                  '" must be a function.';

              if (typeof copy[i].processor !== "string")
                throw 'The processor of key "' +
                  copy[i].key +
                  '" must be a string.';

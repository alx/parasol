...
export default class SigmaComponent extends Component {
  ...
  selectStage() {
    if (
      this.props.appState.filterMode == "singlenode" &&
  ...
  render() {
    ...
    if (appState.ui.renderer == "canvas") {
      ...
      sigmaPlugins.push(
        <EdgeShapes key="sigma-edgeshapes" default={appState.ui.edges.shape} />
      );
    }
    ...
    if (appState.graph.isFiltered && appState.graph.selectedNodes.length > 0) {
      ...
      const selectedNode =
        appState.graph.selectedNodes[appState.graph.selectedNodes.length - 1];
      sigmaPlugins.push(
        <SigmaFilter
          key="sigma-filter"
    ...
    if (options.relativeSize) {
      sigmaPlugins.push(
        <RelativeSize initialSize={15} key="sigma-relativesize" />
      );
    }
      ...
      case "forcelink":
        sigmaPlugins.push(
          <ForceLink
            key="sigma-forcelink"
            appState={appState}
            shouldStart={appState.layout.shouldStart}
            shouldStrop={appState.layout.shouldStop}
            worker={true}
          />
        );
        break;
    ...
    return (
      ...
      >
        ...
        >
          <SigmaLoader graph={graph}>{sigmaPlugins}</SigmaLoader>
        </Sigma>
      </div>
    );
  }

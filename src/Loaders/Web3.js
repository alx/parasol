...
export default class Web3 {
  ...
  onBlock(block) {
    const block_number = parseInt(
      new BN(block.number.replace("0x", ""), 16).toString(10)
    );
    this._createNode(
      block.hash,
      { category: "block", number: block_number },
  ...
  run(callback) {
    ...
    switch (mode) {
      ...
      case "blockstream":
      default:
        this.network.set("categories", [{ name: "block", color: cyan500 }]);
